...
import {Search,Plus,MoreVertical,Mic,Smile,Paperclip,Send,Check,CheckCheck,ArrowLeft,ImageIcon,X,Loader2,Trash2,Pencil,} from "lucide-react";
import { toast } from "sonner";
import { useSearchParams } from "next/navigation";
import { AppImage } from "@/components/ui/AppImage";
import {createCustomWallpaper,createMessageId,getStoredWallpaper,playNotificationSound,storeWallpaper,type ChatMessage,type ChatWallpaper,} from "./messages.helpers";
import {deletePrivateMessage,insertPrivateMessage,loadChatUsers,loadCurrentChatUser,loadPrivateMessages,updatePrivateMessage,uploadChatFileMessage,uploadChatWallpaper,} from "./messages.supabase";
...
const WALLPAPER_PRESETS: ChatWallpaper[] = [
  ...
  },
  {
    id: "rose",
    label: "Rose",
    style: { background: "linear-gradient(135deg, #fff1f2 0%, #fdf4ff 100%)" },
  },
  {
    id: "emerald",
    label: "Emerald",
  ...
  },
  {
    ...
    style: {
      background:
        "linear-gradient(135deg, #fae8ff 0%, #dbeafe 50%, #dcfce7 100%)",
    },
  },
];
...
function MessagesPageContent() {
  ...
  const [wallpaper, setWallpaper] = useState<ChatWallpaper>(
    WALLPAPER_PRESETS[0],
  );
  ...
  const handleWallpaperImageUpload = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > 3 * 1024 * 1024) {
      toast.error("Ảnh quá nặng! Vui lòng chọn ảnh dưới 3MB.");
      return;
    }
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      ...
      if (usersWithAvatars.length > 0) {
        ...
        if (
          requestedChatId &&
          usersWithAvatars.some((u: ChatUser) => u.Id === requestedChatId)
        ) {
          ...
          if (payload.eventType === "INSERT") {
            ...
            if (
              (newMsg.SenderId === currentUser.Id &&
                newMsg.ReceiverId === activeChat) ||
              (newMsg.SenderId === activeChat &&
  ...
  useEffect(() => {
    ...
    const target = messageRefs.current[requestedMessageId];
    if (target) {
    ...
    if (missingMessageToastRef.current !== requestedMessageId) {
      toast.info("Tin nhắn này đã bị xóa hoặc không còn tồn tại.");
      missingMessageToastRef.current = requestedMessageId;
    }
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
  ...
  const handleUpdateMessage = async (e: React.FormEvent, msgId: string) => {
    ...
    setMessages((prev) =>
      prev.map((m) => (m.Id === msgId ? { ...m, Content: editValue } : m)),
    );
                      ...
                      <div className="mt-2 flex items-center gap-2 p-2 bg-slate-50 rounded-xl">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-slate-400">
                            {wallpaper.url.split("/").pop() || "unknown.jpg"}
                          </p>
                        </div>
                        <button
                ...
                messages.map((msg, index) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex flex-col rounded-3xl px-2 py-1 transition-shadow ${
                        requestedMessageId === msg.Id
