...
import { createNewProject, loadProjectCreator, type ProjectCreator } from "./new-project.supabase";
...
export function NewProjectForm() {
  ...
  const [currentUser, setCurrentUser] = useState<ProjectCreator | null>(null);
  const [error, setError] = useState("");

  useEffect(() => {
    let ignore = false;

    loadProjectCreator().then(({ user, redirectTo }) => {
      if (ignore) return;
      if (redirectTo) {
