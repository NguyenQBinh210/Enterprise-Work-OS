        ...
        <div className="flex min-w-0 items-center gap-3.5">
          <span
            className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-all duration-200 ${
              isActive ? 'bg-white/15 text-white' : 'text-slate-400 group-hover/link:bg-white/[0.06] group-hover/link:text-blue-200'
            }`}
          >
  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative z-10 flex h-20 shrink-0 items-center justify-between border-b border-white/10 px-4 md:h-[88px] md:px-4">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0 whitespace-nowrap opacity-100 transition-all duration-300 md:translate-x-1 md:opacity-0 md:group-hover:translate-x-0 md:group-hover:opacity-100">
              <p className="truncate text-xl font-bold tracking-tight text-white">TaskFlow</p>
              <p className="truncate text-[11px] font-semibold text-blue-200/80">Enterprise Work OS</p>
            </div>
          </div>
        ...
        <div className="relative z-10 shrink-0 border-t border-white/10 p-3 md:p-4">
          <div className="flex items-center gap-3 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-2 transition-colors duration-200 hover:bg-white/[0.07]">
            ...
            <div className="min-w-0 flex-1 whitespace-nowrap opacity-100 transition-all duration-300 md:translate-x-1 md:opacity-0 md:group-hover:translate-x-0 md:group-hover:opacity-100">
              <p className="truncate text-sm font-semibold text-white">{user ? user.FullName : '...'}</p>
              <span className={`mt-1 inline-flex rounded-md px-1.5 py-0.5 text-[9px] font-bold uppercase ${
                isAdmin ? 'bg-violet-400/15 text-violet-200' : isManager ? 'bg-blue-400/15 text-blue-200' : 'bg-slate-700 text-slate-300'
              }`}>
                {role || 'USER'}
              </span>
            </div>
            ...
            <div className="flex shrink-0 items-center gap-1 opacity-100 transition-opacity duration-300 md:opacity-0 md:group-hover:opacity-100">
              <Link href="/dashboard/profile" className="rounded-lg p-2 text-slate-400 transition-colors hover:bg-blue-500/10 hover:text-blue-200">
                <Settings className="h-4 w-4" />
              </Link>
              <button onClick={handleLogout} className="rounded-lg p-2 text-slate-400 transition-colors hover:bg-rose-500/10 hover:text-rose-200">
